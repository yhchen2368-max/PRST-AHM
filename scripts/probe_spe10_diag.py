"""SPE10 Model 2 first-step diagnostic.

mrst_drs + trueIMPES, tolerance=1e-3, maxTimestepCuts=16, full Schur, with
per-Newton / per-mini-step progress streamed so we can see exactly where it
stalls (linear solve not converging vs Newton not reducing).
"""
import sys
import threading
import time
...
linear = AMGCL_CPRSolverBlockAD(tolerance=1e-3, maxIterations=50, verbose=False,
                                strategy='mrst_drs', decoupling='trueIMPES',
                                schurApproxType='full')
lin = {'n': 0, 'iters': 0, 't': 0.0, 'prep': 0.0, 'kernel': 0.0}
