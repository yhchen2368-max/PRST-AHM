...
byte-frozen extracted source ``FAHM.m``.  The window, tab hierarchy,
    ...
    navigation callbacks follow ``createComponents``/``startupFcn``.  Create
    Project's baseCase/writeDeck/external-command preflight follows
    ``CreatProjectButtonPushed`` through its simulator call.
...
APP_SIZE = (915, 736)
# User-authorized PRST convenience deviation from FAHM's ``pause(20)``.
...
class FahmApp(ttk.Frame):
    ...
    def _finish_startup(self):
        """Perform the visibility transition after PRST's two-second splash."""
        self._startup_after_id = None
    ...
    def _creat_project(self):
        """Port ``CreatProjectButtonPushed`` through FAHM.m:1824."""
        ...
        try:
            ...
            messagebox.showerror(
                'ERROR', 'Failed in importing the Eclipse data.')
            self.deck = None
            return
        ...
        try:
            result = create_base_case(
                self.deck, self.ModelPath.get(), self.SimulatorPath.get())
        except Exception as exc:
            failed = getattr(exc, 'result', None)
            self.baseCase = getattr(failed, 'base_case',
                                    getattr(exc, 'base_case', ''))
            messagebox.showerror('ERROR', str(exc))
            self._say('Create Project failed: %s' % exc)
            return

        self.base_case_result = result
        self.baseCase = result.base_case
        try:
            project = initialize_fahm_project(self.deck, result.base_case)
        except Exception as exc:
            messagebox.showerror('ERROR', str(exc))
            self._say('Create Project state setup failed: %s' % exc)
            return

        self.deck = project.deck
        self.G = project.G
        self.rock = project.rock
        self.fluid = project.fluid
        self.state0 = project.state0
        self.model = project.model
        self.N = project.N
        self.T = project.T
        self._say('Project created: %s' % result.base_case)
